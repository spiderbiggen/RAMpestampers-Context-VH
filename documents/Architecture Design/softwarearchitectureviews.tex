...
In this section, the way GOAL is used in our agent will be explained.\\

Our agent works with submodules and one main module. The main module calls the other modules when needed. The other modules are the buildConstruction and the demolish modules. The buildConstruction module is called when there is an indicator goal to build high buildings. In this module, either a building is build or a goal is adopted to create land.  The demolish module is called when there is a goal to create land or an indicator goal to demolish buildings. The demolish module demolishes a building from the list of old buildings.
